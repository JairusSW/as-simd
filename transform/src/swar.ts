...
const raw = binaryen as unknown as {
  _BinaryenBinary(module: number, op: BinaryOp, left: ExpressionRef, right: ExpressionRef): ExpressionRef;
};
...
class SwarRewriter extends ExpressionRewriter {
  ...
  private fuseZeroLaneComparison(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
  ...
  private fuseZeroLaneReduction(expr: ExpressionRef, width: Width): ExpressionRef | null {
    const outer = valueAndConstant(expr, width);
    if (!outer || info(outer.value).id !== binaryen.BinaryId || binary(outer.value).op !== andOp(width)) return null;
    const { left, right } = binary(outer.value);
    for (const [difference, inverse] of [[left, right], [right, left]] as const) {
      if (info(difference).id !== binaryen.BinaryId || binary(difference).op !== subOp(width) || !isComplement(inverse, width)) continue;
      const differenceNode = binary(difference);
      const ones = integerConstantValue(differenceNode.right, width);
      if (ones === null || !sameValueReference(differenceNode.left, complementValue(inverse))) continue;
      const broadcast = laneBroadcast(differenceNode.left, width);
      if (!broadcast || outer.constant !== broadcast.highMask || ones !== (broadcast.highMask >> BigInt(broadcast.shift))) continue;
      const target = broadcast.complemented ? 0n : broadcast.highMask;
      if (target === 0n) {
        const source = laneNonzeroSource(broadcast.highBits, width);
        if (source) return this.changed(makeBinary(this.module, eqOp(width), source, integerConstant(this.module, width, 0n)));
      }
   ...
   */
  private fuseExpandedLaneReduction(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
    ...
    if (detector && pair.constant === detector.highMask) {
      ...
      if (hasZero) return this.changed(makeBinary(this.module, op, hasZero, integerConstant(this.module, width, 0n)));
    }
    ...
    if (target === 0n) {
      const source = laneNonzeroSource(broadcast.highBits, width);
      if (source) return this.changed(makeBinary(this.module, op, source, integerConstant(this.module, width, 0n)));
    }
   ...
   */
  private fuseExpandedLaneMask(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
    ...
    if (info(expanded).id !== binaryen.BinaryId || binary(expanded).op !== mulOp(width)) return null;
    ...
    if (shift === 0 || !isLaneBroadcast(outer.constant, shift, product.constant, width)) return null;
  ...
  private mergeConstantMasks(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
    const isAdd = op === addOp(width);
    if (op !== orOp(width) && op !== xorOp(width) && op !== andOp(width) && !isAdd) return null;

    const left = binary(expr).left;
    const right = binary(expr).right;
    if (info(left).id !== binaryen.BinaryId || info(right).id !== binaryen.BinaryId) return null;
   ...
   */
  private factorCommonMask(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
    if (op !== andOp(width) && op !== orOp(width) && op !== xorOp(width)) return null;
    const left = binary(expr).left;
    const right = binary(expr).right;
    if (info(left).id !== binaryen.BinaryId || info(right).id !== binaryen.BinaryId) return null;
    const childOp = binary(left).op;
    if ((childOp !== andOp(width) && childOp !== orOp(width)) || binary(right).op !== childOp) return null;
  ...
  private fuseShiftMasks(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
    ...
    if (innerAmount === null || Number(innerAmount & BigInt(width - 1)) !== shift) return null;
   ...
   */
  private fuseBitselect(expr: ExpressionRef, op: BinaryOp, width: Width): ExpressionRef | null {
    ...
    if (!left || !right || left.complemented === right.complemented) return null;

    const plain = left.complemented ? right : left;
    const inverse = left.complemented ? left : right;
    if (!sameSimpleValue(plain.mask, inverse.mask) || !isCheapLeaf(plain.value) || !isCheapLeaf(inverse.value)) return null;
 ...
 */
function makeHasZero(module: Module, source: ExpressionRef, highMask: bigint, shift: number, width: Width): ExpressionRef | null {
  let second: ExpressionRef;
  const sourceInfo = info(source);
  if (sourceInfo.id === binaryen.LocalSetId && (sourceInfo as binaryen.LocalSetInfo).isTee) {
...
function isLaneBroadcast(highMask: bigint, shift: number, laneMask: bigint, width: Width): boolean {
  const all = (1n << BigInt(width)) - 1n;
  let occupied = 0n;
  for (let bitIndex = 0; bitIndex < width; bitIndex++) {
    ...
    if ((expansion & highMask) !== bit || (occupied & expansion) !== 0n) return false;
    occupied |= expansion;
  }
  return occupied === all;
}
...
function laneBroadcast(expr: ExpressionRef, width: Width): LaneBroadcast | null {
  let expanded = expr;
  const expandedInfo = info(expanded);
  if (expandedInfo.id === binaryen.LocalSetId && (expandedInfo as binaryen.LocalSetInfo).isTee) {
  ...
  const shift = Number(shiftValue & BigInt(width - 1));
  const high = valueAndConstant(shiftNode.left, width);
  if (!high || shift === 0 || !isLaneBroadcast(high.constant, shift, product.constant, width)) return null;
...
function laneNonzeroSource(expr: ExpressionRef, width: Width): ExpressionRef | null {
  return laneNonzeroDetector(expr, width)?.source ?? null;
}

function laneNonzeroDetector(expr: ExpressionRef, width: Width): LaneNonzeroDetector | null {
  const outer = valueAndConstant(expr, width);
  if (!outer || info(outer.value).id !== binaryen.BinaryId || binary(outer.value).op !== orOp(width)) return null;
  const all = (1n << BigInt(width)) - 1n;
  const lowMask = all ^ outer.constant;
  const shift = trailingZeroes(outer.constant, width);
  const laneMask = (1n << BigInt(shift + 1)) - 1n;
  if (shift === 0 || !isLaneBroadcast(outer.constant, shift, laneMask, width)) return null;
  const { left, right } = binary(outer.value);
  for (const [sumExpr, source] of [[left, right], [right, left]] as const) {
...
function integerConstantValue(expr: ExpressionRef, width: Width): bigint | null {
...
function integerConstant(module: Module, width: Width, value: bigint): ExpressionRef {
  if (width === 32) return module.i32.const(Number(value & 0xffff_ffffn) | 0);
  return module.i64.const(BigInt.asIntN(64, value));
}

function makeBinary(module: Module, op: BinaryOp, left: ExpressionRef, right: ExpressionRef): ExpressionRef {
  return raw._BinaryenBinary(module.ptr, op, left, right);
}

function valueAndConstant(expr: ExpressionRef, width: Width): { value: ExpressionRef; constant: bigint } | null {
  if (info(expr).id !== binaryen.BinaryId || binary(expr).op !== andOp(width)) return null;
...
function valueAndConstantOp(expr: ExpressionRef, width: Width, op: BinaryOp): { value: ExpressionRef; constant: bigint } | null {
  if (info(expr).id !== binaryen.BinaryId || binary(expr).op !== op) return null;
  const { left, right } = binary(expr);
  const rc = integerConstantValue(right, width);
  if (rc !== null) return { value: left, constant: rc };
  const lc = integerConstantValue(left, width);
...
function isComplement(expr: ExpressionRef, width: Width): boolean {
  if (info(expr).id !== binaryen.BinaryId || binary(expr).op !== xorOp(width)) return false;
  const all = (1n << BigInt(width)) - 1n;
...
function sameSimpleValue(a: ExpressionRef, b: ExpressionRef): boolean {
  ...
  switch (aid) {
    case binaryen.LocalGetId:
      return (ai as binaryen.LocalGetInfo).index === (bi as binaryen.LocalGetInfo).index;
    case binaryen.GlobalGetId:
      return (ai as binaryen.GlobalGetInfo).name === (bi as binaryen.GlobalGetInfo).name;
    case binaryen.ConstId:
      return (ai as binaryen.ConstInfo).value === (bi as binaryen.ConstInfo).value;
    default:
      return false;
  }
}
...
function sameValueReference(a: ExpressionRef, b: ExpressionRef): boolean {
  ...
  if ((ai.id === binaryen.LocalGetId || ai.id === binaryen.LocalSetId) && (bi.id === binaryen.LocalGetId || bi.id === binaryen.LocalSetId)) {
    ...
    return aLocal.index === bLocal.index && (ai.id !== binaryen.LocalSetId || (ai as binaryen.LocalSetInfo).isTee) && (bi.id !== binaryen.LocalSetId || (bi as binaryen.LocalSetInfo).isTee);
  }
