...
function instructionName(bits: number, subopcode: number): string {
  ...
  if (!base)
    throw new Error(
      `as-simd: missing semantic name for SIMD opcode ${subopcode}`,
    );
...
class KernelRewriter extends ExpressionRewriter {
  ...
  constructor(module: binaryen.Module) {
    ...
    for (let i = 0, n = module.getNumFunctions(); i < n; i++) {
      this.originalFunctions.push(
        binaryen.getFunctionInfo(module.getFunctionByIndex(i)).name,
      );
    }
  }
  ...
  protected rewrite(expr: Ref): Ref {
    ...
    for (let i = 0; i + 1 < children.length; i++) {
      ...
      const genericCalls =
        matchV256KernelCallPair(children, i) ??
      ...
      if (calls) {
        raw._BinaryenBlockSetChildAt(
          expr,
          i,
          this.kernel(512, calls.subopcode, calls.op, "binary", calls.dst, [
            calls.left,
            calls.right,
          ]),
        );
      ...
      if (upper) {
        raw._BinaryenBlockSetChildAt(
          expr,
          i,
          this.kernel(512, upper.subopcode, upper.op, "binary", upper.dst, [
            upper.left,
            upper.right,
          ]),
        );
  ...
  private helper(
    ...
    const params = binaryen.createType(
      Array.from({ length: arity }, () => this.carrier),
    );
    this.module.addFunctionImport(
      name,
      INSTRUCTION_MODULE,
      instructionName(bits, subopcode),
      params,
      this.carrier,
    );
  ...
  private kernel(
    ...
    const values = inputs.map((input) =>
      this.module.call(load, [this.module.i32.const(input)], this.carrier),
    );
    const result = this.module.call(
      this.helper(bits, subopcode, op, kind),
      values,
      this.carrier,
    );
    return this.module.call(
      this.store(bits),
      [result, this.module.i32.const(dst)],
      binaryen.none,
    );
  }
}
...
function operationForSubopcode(
  subopcode: number,
): { kind: KernelKind; op: number } | null {
...
function v256KernelCall(expr: Ref): KernelStore | null {
  ...
  if (
    call.target === "__as_simd_instruction_v256_store" &&
    call.operands.length === 2
  ) {
    const operationCall = safeInfo(
      call.operands[0],
    ) as binaryen.CallInfo | null;
    const dst = constantAddress(call.operands[1], 0);
    if (!operationCall || operationCall.id !== binaryen.CallId || dst === null)
      return null;
    const match = /^__as_simd_instruction_v256_fd_(\d+)$/.exec(
      operationCall.target,
    );
    if (!match) return null;
    const subopcode = Number(match[1]),
      operation = operationForSubopcode(subopcode);
    if (!operation) return null;
    const arity =
      operation.kind === "unary" ? 1 : operation.kind === "binary" ? 2 : 3;
  ...
  return {
    ...
    op: operation.op,
    subopcode,
    dst: addresses[0] as number,
    inputs: addresses.slice(1) as number[],
  };
}

function matchV256KernelCallPair(
  children: Ref[],
  index: number,
): (KernelStore & { consumed: number }) | null {
...
function matchV256KernelCallAndUpperPair(
  children: Ref[],
  index: number,
): (KernelStore & { consumed: number }) | null {
  const first = v256KernelCall(children[index]);
  ...
  while (
    ...
    next++;
  const upperFirst =
    next < children.length ? kernelStore(children[next]) : null;
  if (!upperFirst || !sameKernelStride(first, upperFirst, 32)) return null;
  const last = next + 1,
    rawStore =
      last < children.length
        ? (safeInfo(children[last]) as binaryen.StoreInfo | null)
        : null;
  if (
    !rawStore ||
...
function operationInfo(expr: Ref): { kind: KernelKind; op: number } | null {
  ...
  if (value.id === binaryen.UnaryId)
    return { kind: "unary", op: (value as binaryen.UnaryInfo).op };
  if (value.id === binaryen.BinaryId)
    return { kind: "binary", op: (value as binaryen.BinaryInfo).op };
  if (value.id === binaryen.SIMDTernaryId)
    return { kind: "ternary", op: (value as binaryen.SIMDTernaryInfo).op };
  return null;
}

function matchV256CallPair(
  children: Ref[],
  index: number,
): {
  subopcode: number;
...
function matchV256CallAndUpperPair(
  children: Ref[],
  index: number,
): {
  subopcode: number;
  ...
  while (
    ...
    next++;
  const upperFirst =
    next < children.length ? binaryStore(children[next]) : null;
  if (
    !upperFirst ||
  ...
  const rawValue =
    rawStore?.id === binaryen.StoreId
      ? (safeInfo(rawStore.value) as binaryen.BinaryInfo | null)
      : null;
  const rawDst =
    rawStore?.id === binaryen.StoreId
...
function v256Call(expr: Ref): {
  ...
  const dst = constantAddress(call.operands[0], 0),
    left = constantAddress(call.operands[1], 0),
    right = constantAddress(call.operands[2], 0);
...
function matchKernelPair(a: Ref, b: Ref): KernelStore | null {
  const first = kernelStore(a),
    second = kernelStore(b);
  return first && second && sameKernelStride(first, second, 16) ? first : null;
}

function matchKernelSequence(
  children: Ref[],
  index: number,
  count: number,
): (KernelStore & { consumed: number }) | null {
  ...
  const parts = Array.from({ length: count }, (_, i) =>
    partialKernelStore(children[index + i]),
  );
  const first = parts[0];
  if (!first) return null;
...
function partialKernelStore(
  expr: Ref,
): (Omit<KernelStore, "inputs"> & { inputs: (number | null)[] }) | null {
  ...
  const dst = constantAddress(store.ptr, store.offset),
    info = operationInfo(store.value);
  ...
  if (info.kind === "unary") {
    ...
    operands = [(value as binaryen.UnaryInfo).value];
...
function sameKernelStride(
  ...
  return (
    ...
    first.inputs.every(
      (input, index) => second.inputs[index] === input + stride,
    )
  );
}
...
function kernelStore(expr: Ref): KernelStore | null {
  ...
  return inputs.some((input) => input === null)
    ? null
    : { kind, op, subopcode, dst, inputs: inputs as number[] };
}

function matchBinaryPair(
  a: Ref,
  b: Ref,
): {
  subopcode: number;
  ...
  const first = binaryStore(a),
    second = binaryStore(b);
  if (
    !first ||
    !second ||
    first.op !== second.op ||
...
function binaryStore(
  expr: Ref,
): { op: number; dst: number; left: number; right: number } | null {
  const store = safeInfo(expr) as binaryen.StoreInfo | null;
  if (
    !store ||
    store.id !== binaryen.StoreId ||
  ...
  if (
    ...
    return null;
  const left = vectorLoad(value.left),
    right = vectorLoad(value.right),
    dst = constantAddress(store.ptr, store.offset);
...
function safeInfo(expr: Ref): binaryen.ExpressionInfo | null {
  try {
