...
export function computeImportBaseRel(
  fromDir: string,
  packageDir: string,
  flavor: PathFlavor = path,
): string {
  const relative = flavor
    .relative(fromDir, packageDir)
    .split(flavor.sep)
    .join("/");
...
export function packageRootFromTransform(
 ...
 */
export function computeVectorImportSpecifier(
  normalizedSourcePath: string,
  baseDir: string,
  packageDir: string = packageRootFromTransform(),
): string {
  const cwd = path.resolve(process.cwd(), baseDir);
  const sourcePath = normalizedSourcePath.replaceAll("/", path.sep);
  const fromFile = path.isAbsolute(sourcePath)
    ? sourcePath
    : path.resolve(
        cwd,
        sourcePath.startsWith(`~lib${path.sep}`)
          ? sourcePath.slice(5)
          : sourcePath,
      );
