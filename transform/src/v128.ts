...
export function injectPortableVectors(
  parser: Parser,
  injectV128: boolean,
  baseDir: string,
): number {
  ...
  for (const source of parser.sources) {
    ...
    const needed = VECTOR_NAMES.filter(
      (name) =>
        (name !== "v128" || injectV128) &&
        !declared.has(name) &&
        new RegExp(`\\b${name}\\b`).test(source.text),
    );
    if (!needed.length) continue;
    const specifier = computeVectorImportSpecifier(
    ...
    source.statements.unshift(
      parseImport(
        parser,
        `import { ${needed.join(", ")} } from "${specifier}";`,
        source,
      ),
    );
    rewritten++;
...
function isAsSimdSource(path: string): boolean {
  ...
  return (
    normalized.startsWith("assembly/") ||
...
function declaredNames(source: Source): Set<VectorName> {
  ...
  for (const statement of source.statements) {
    ...
    if (VECTOR_NAMES.includes(named.name?.text as VectorName))
      declared.add(named.name!.text as VectorName);
    if (statement instanceof ImportStatement) {
      if (VECTOR_NAMES.includes(statement.namespaceName?.text as VectorName))
        declared.add(statement.namespaceName!.text as VectorName);
      for (const declaration of statement.declarations ?? []) {
        if (VECTOR_NAMES.includes(declaration.name.text as VectorName))
          declared.add(declaration.name.text as VectorName);
        if (VECTOR_NAMES.includes(declaration.foreignName.text as VectorName))
          declared.add(declaration.foreignName.text as VectorName);
      }
    }
  }
  return declared;
