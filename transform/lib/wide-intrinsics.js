...
function operationForSubopcode(subopcode) {
    for (const [kind, map] of [["unary", unarySubopcode], ["binary", binarySubopcode], ["ternary", ternarySubopcode]]) {
...
function v256KernelCall(expr) {
    ...
    if (call.target === "__as_simd_instruction_v256_store" && call.operands.length === 2) {
        ...
        return inputs.some(input => input === null)
            ? null
            : { kind: operation.kind, op: operation.op, subopcode, dst, inputs: inputs };
    }
    ...
    if (addresses.some(address => address === null))
        return null;
    return { kind: operation.kind, op: operation.op, subopcode, dst: addresses[0], inputs: addresses.slice(1) };
}
function matchV256KernelCallPair(children, index) {
    ...
    if (!first)
        return null;
    let next = index + 1;
    while (next < children.length && safeInfo(children[next])?.id === binaryen.NopId)
        next++;
...
function matchV256KernelCallAndUpperPair(children, index) {
    ...
    if (!upperFirst || !sameKernelStride(first, upperFirst, 32))
        return null;
    const last = next + 1, rawStore = last < children.length ? safeInfo(children[last]) : null;
    if (!rawStore || rawStore.id !== binaryen.StoreId || rawStore.bytes !== 16 || rawStore.isAtomic || constantAddress(rawStore.ptr, rawStore.offset) !== first.dst + 48)
        return null;
...
function matchV256CallPair(children, index) {
    ...
    if (!second || first.subopcode !== second.subopcode || second.dst !== first.dst + 32 || second.left !== first.left + 32 || second.right !== first.right + 32)
        return null;
...
function matchV256CallAndUpperPair(children, index) {
    ...
    if (!upperFirst || upperFirst.op !== first.op || upperFirst.dst !== first.dst + 32 || upperFirst.left !== first.left + 32 || upperFirst.right !== first.right + 32)
        return null;
    const last = next + 1;
    const rawStore = last < children.length ? safeInfo(children[last]) : null;
    const rawValue = rawStore?.id === binaryen.StoreId ? safeInfo(rawStore.value) : null;
    const rawDst = rawStore?.id === binaryen.StoreId ? constantAddress(rawStore.ptr, rawStore.offset) : null;
    if (!rawStore || rawStore.bytes !== 16 || rawStore.isAtomic || !rawValue || rawValue.id !== binaryen.BinaryId || rawValue.op !== first.op || rawDst !== first.dst + 48)
        return null;
...
function matchKernelSequence(children, index, count) {
    ...
    for (let i = 1; i < count; i++) {
        const part = parts[i];
        if (!part || part.kind !== first.kind || part.op !== first.op || part.subopcode !== first.subopcode || part.dst !== first.dst + i * 16 || part.inputs.length !== first.inputs.length)
            return null;
    }
    const inputs = [];
...
function partialKernelStore(expr) {
    ...
    if (!store || store.id !== binaryen.StoreId || store.bytes !== 16 || store.isAtomic)
        return null;
...
function kernelStore(expr) {
    ...
    return inputs.some(input => input === null) ? null : { kind, op, subopcode, dst, inputs: inputs };
}
function matchBinaryPair(a, b) {
    const first = binaryStore(a), second = binaryStore(b);
    if (!first || !second || first.op !== second.op || second.dst !== first.dst + 16 || second.left !== first.left + 16 || second.right !== first.right + 16)
        return null;
...
function binaryStore(expr) {
    ...
    if (!store || store.id !== binaryen.StoreId || store.bytes !== 16 || store.isAtomic)
        return null;
    const value = safeInfo(store.value);
    if (!value || value.id !== binaryen.BinaryId || !binarySubopcode.has(value.op))
        return null;
