...
function compile(name, enabled) {
  ...
  execFileSync("node_modules/.bin/asc", ["transform/__tests__/fixture.ts", "--config", "none", "-O0", "--noAssert", "--transform", "./transform", "-o", `${output}/${name}.wasm`, "--textFile", `${output}/${name}.wat`], { env, stdio: "inherit" });
}
...
function functionBody(name) {
  return wat.match(new RegExp(`\\(func \\$[^\\s()]*${name}(?=\\s)[\\s\\S]*?\\n \\)`))?.[0] ?? "";
}
...
assert.equal((indirectBody.match(/i64\.and/g) ?? []).length, 1, "call_indirect operand did not fuse bitselect");
...
assert.equal((functionBody("xorOverlappingMasks").match(/i64\.and/g) ?? []).length, 1);
assert.doesNotMatch(functionBody("xorOverlappingMasks"), /i64\.xor/);
assert.equal((functionBody("factorAndMask").match(/i64\.and/g) ?? []).length, 1);
assert.equal((functionBody("factorOrMask").match(/i64\.or/g) ?? []).length, 0);
assert.equal((functionBody("mismatchedMasks").match(/i64\.and/g) ?? []).length, 2);
for (const name of ["anyEq8", "anyNe8", "allEq8", "allNe8", "anyEq16", "anyNe16", "allEq16", "allNe16", "anyEq32", "anyNe32", "allEq32", "allNe32"]) {
  const body = functionBody(name);
  assert.ok(body, `${name} body not found`);
  assert.doesNotMatch(body, /i64\.mul/, `${name} still expands a compact lane mask`);
}
for (const name of ["anyEq8", "allNe8", "anyEq16", "allNe16", "anyEq32", "allNe32"]) {
  const body = functionBody(name);
  assert.match(body, /i64\.sub/, `${name} does not use the reduced has-zero predicate`);
  assert.doesNotMatch(body, /i64\.add/, `${name} still builds an exact nonzero-lane mask`);
  assert.doesNotMatch(body, /i64\.or/, `${name} still builds an exact nonzero-lane mask`);
}

const imports = { env: { abort() { throw new Error("unexpected abort"); } } };
const optimized = await WebAssembly.instantiate(readFileSync(`${output}/optimized.wasm`), imports);
const plain = await WebAssembly.instantiate(readFileSync(`${output}/plain.wasm`), imports);
...
for (let i = 0; i < 100_000; i++) {
  ...
  assert.equal(a.bitselect32(x32, y32, mask32), b.bitselect32(x32, y32, mask32));
