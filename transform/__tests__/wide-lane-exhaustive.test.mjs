...
const widths = [
  {bits: 256, vector: "v256", path: "assembly/v256/lanes.ts", namespaces: ["i8x32", "i16x16", "i32x8", "i64x4", "f32x8", "f64x4"]},
  {bits: 512, vector: "v512", path: "assembly/v512/lanes.ts", namespaces: ["i8x64", "i16x32", "i32x16", "i64x8", "f32x16", "f64x8"]},
];
...
function namespaceFunctions(source, namespace) {
  ...
  return [...body.matchAll(/export function\s+(\w+)\s*\(([^)]*)\)\s*:\s*([^\s{]+)/g)].map(match => ({
    name: match[1],
    params: match[2].split(",").map(value => value.trim()).filter(Boolean).map(value => {
      const split = value.indexOf(":");
      return {name: value.slice(0, split).trim(), type: value.slice(split + 1).trim()};
    }),
    result: match[3],
  }));
}
...
for (const width of widths) {
  ...
  for (const namespace of width.namespaces) {
    ...
    lines.push(`  const ${namespace}a = ${namespace}.splat(${scalarLiteral(laneType, 0)});`);
    lines.push(`  const ${namespace}b = ${namespace}.splat(${scalarLiteral(laneType, 1)});`);
    lines.push(`  const ${namespace}m = ${namespace}.splat(${scalarLiteral(laneType, 2)});`);
    let namespaceCall = 0;
...
const imported = [...imports].filter(name => name !== "v256" && name !== "v512");
lines[0] = `import { v256, v512, ${imported.join(", ")} } from "../../assembly";`;
assert.equal(calls, 462, "every current lane-namespace function must have one generated execution case");
...
function compile(mode) {
  ...
  const args = [fixture, "--config", "none", "--runtime", "stub", "--noAssert", "-O0", "--exportRuntime", "-o", wasm];
...
async function execute(bytes) {
  ...
  return new Map(cases.map(({name, functions}) => [name, functions.map((_, index) => instance.exports[`run_${name}`](index + 1))]));
}
...
for (const {name, functions} of cases) {
  for (let i = 0; i < functions.length; i++) {
    const actual = simd.get(name)[i], expected = swar.get(name)[i];
    assert.equal(actual, expected, `${name}.${functions[i]} SIMD/SWAR checksum differs: simd=${actual.toString(16)} swar=${expected.toString(16)}`);
  }
}
console.log(`exhaustive v256/v512 lane namespace parity passed (${calls} operations)`);
