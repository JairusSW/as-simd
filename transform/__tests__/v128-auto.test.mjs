...
function compile(name, simd) {
  const args = [fixture, "--config", "none", "-O0", "--noAssert", "--transform", "./transform", "--initialMemory", "1", "-o", `${output}/${name}.wasm`, "--textFile", `${output}/${name}.wat`];
...
function compileVectors(name, fixtureName, simd) {
  const args = [`transform/__tests__/${fixtureName}`, "--config", "none", "-O0", "--noAssert", "--transform", "./transform", "--initialMemory", "1", "-o", `${output}/${name}.wasm`, "--textFile", `${output}/${name}.wat`];
  if (simd) args.push("--enable", "simd");
  execFileSync("node_modules/.bin/asc", args, { stdio: "inherit" });
}
...
const imports = { env: { abort() { throw new Error("fallback aborted"); } } };
const fallback = (await WebAssembly.instantiate(readFileSync(`${output}/v128-fallback.wasm`), imports)).instance;
const native = (await WebAssembly.instantiate(readFileSync(`${output}/v128-native.wasm`), imports)).instance;

for (let a = -128; a < 128; a += 7) {
  for (let b = -128; b < 128; b += 11) {
    assert.equal(fallback.exports.autoV128(a, b), native.exports.autoV128(a, b));
  }
}
...
for (let i = 0; i < 1_000; i++) {
  ...
  assert.equal(fallback.exports.autoV128Memory(0, i | 0, (i * 13) | 0), native.exports.autoV128Memory(0, i | 0, (i * 13) | 0));
  assert.deepEqual([...new Uint32Array(fallback.exports.memory.buffer, 0, 4)], [...new Uint32Array(native.exports.memory.buffer, 0, 4)]);
}
...
const vectorModules = {};
for (const name of ["vectors-auto-swar", "vectors-auto-simd", "vectors-import-swar", "vectors-import-simd"]) {
  vectorModules[name] = (await WebAssembly.instantiate(readFileSync(`${output}/${name}.wasm`), imports)).instance.exports;
}
assert.doesNotMatch(readFileSync(`${output}/vectors-auto-swar.wat`, "utf8"), /v128\.|i32x4\./);
assert.match(readFileSync(`${output}/vectors-auto-simd.wat`, "utf8"), /i32x4\./);
function functionBody(wat, name) {
  const start = wat.indexOf(`(func $transform/__tests__/vectors-auto-fixture/${name} `);
...
for (const mode of ["swar", "simd"]) {
  ...
  const allocations = (name) => (functionBody(wat, name).match(/call \$~lib\/rt\/itcms\/__new/g) ?? []).length;
  assert.equal(allocations("v256Checksum"), 5, `dedicated v256 ${mode} value chain regressed`);
  assert.equal(allocations("v512Checksum"), 5, `dedicated v512 ${mode} value chain regressed`);
...
console.log("explicit and auto-injected generic/lane vector namespace parity tests passed");
