...
function compile(mode) {
  const args = [
    ...
    "--converge",
    "-o",
    `${output}/wide-${mode}.wasm`,
    "--textFile",
    `${output}/wide-${mode}.wat`,
  ];
  if (mode === "simd" || mode === "native" || mode === "relaxed")
...
assert.doesNotMatch(
  simd,
  /\(import "as-simd" "(?:v256|v512)\./,
  "ordinary SIMD builds must not require Wide",
);
assert.match(
  body(native, "v256AddI8"),
  /call \$__as_simd_instruction_v256_fd_110/,
);
assert.match(native, /\(import "as-simd" "i8x32\.add"/);
assert.match(
  native,
  /\(import "as-simd" "v256\.load" \(func \$[^ ]+ \(param i32\) \(result externref\)\)\)/,
);
assert.match(
  native,
  /\(import "as-simd" "i8x32\.add" \(func \$[^ ]+ \(param externref externref\) \(result externref\)\)\)/,
);
assert.match(
  native,
  /\(import "as-simd" "v256\.store" \(func \$[^ ]+ \(param externref i32\)\)\)/,
);
assert.match(
  body(native, "v256AddI8"),
  /call \$__as_simd_instruction_v256_load[\s\S]*call \$__as_simd_instruction_v256_fd_110[\s\S]*call \$__as_simd_instruction_v256_store/,
);
assert.match(
  body(native, "v512AddI8"),
  /call \$__as_simd_instruction_v512_fd_110/,
);
...
assert.match(
  body(native, "v512NegI32"),
  /call \$__as_simd_instruction_v512_fd_161/,
);
assert.match(native, /\(import "as-simd" "i32x16\.neg"/);
assert.doesNotMatch(native, /\(import "[^"]*" "v(?:256|512)\.fd\./);
const nativeModule = new WebAssembly.Module(
  readFileSync(`${output}/wide-native.wasm`),
);
assert.equal(
  WebAssembly.Module.customSections(
    nativeModule,
    "metadata.code.wago.intrinsics",
  ).length,
  0,
);
for (const wat of [swar, simd, relaxed]) {
  assert.doesNotMatch(
    wat,
    /\$assembly\/(?:wide|v128)\/regfile\/(?:w?rf)\._base/,
    "fixed register files must not reintroduce lazy base globals",
  );
}
...
for (const name of [
  ...
  assert.equal(
    (body(swar, name).match(/\n  loop /g) ?? []).length,
    2,
    `${name} SWAR kernel was not fully unrolled`,
  );
  assert.equal(
    (body(simd, name).match(/\n  loop /g) ?? []).length,
    2,
    `${name} SIMD kernel was not fully unrolled`,
  );
}
...
for (const name of ["v256And", "v256Or", "v256Xor", "v256Not"]) {
  assert.doesNotMatch(
    body(simd, name),
    /v128\.(?:and|or|xor|not)/,
    `${name} must retain the measured scalar-word path`,
  );
}
assert.equal(
  (body(simd, "v256MulI8").match(/i16x8\.extmul_low_i8x16_u/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v256MulI8").match(/i16x8\.extmul_high_i8x16_u/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v256MulI8").match(/i8x16\.shuffle/g) ?? []).length,
  2,
);
...
for (const [name, opcode] of [
  ["v256NegI64", /i64x2\.neg/],
  ["v256AbsI64", /i64x2\.abs/],
  ["v256ShlI64", /i64x2\.shl/],
  ["v256ShrI64", /i64x2\.shr_s/],
]) {
  assert.doesNotMatch(
    body(simd, name),
    opcode,
    `${name} must retain the measured scalar path`,
  );
}
assert.equal(
  (body(simd, "v256SplatI64").match(/i64x2\.splat/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v256SplatI64").match(/v128\.store/g) ?? []).length,
  2,
);
...
for (const [name, opcode] of [
  ["v256NegI8", /i8x16\.neg/],
  ["v256ShlI8", /i8x16\.shl/],
  ["v256ShrI8", /i8x16\.shr_s/],
  ["v256NegI16", /i16x8\.neg/],
  ["v256ShlI16", /i16x8\.shl/],
]) {
  assert.doesNotMatch(
    body(simd, name),
    opcode,
...
assert.equal(
  (body(simd, "v256ReplaceLane").match(/v128\.load/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v256ReplaceLane").match(/v128\.store/g) ?? []).length,
  2,
);
...
for (const mode of [swar, simd]) {
  ...
  assert.doesNotMatch(
    lane,
    /v128\.(?:load|store)|i64\.(?:shr|shl)/,
    "v256 lane extraction must remain one direct scalar load",
  );
}
for (const name of [
  "v512AddI8",
...
assert.equal(
  (body(simd, "v512Bitselect").match(/v128\.bitselect/g) ?? []).length,
  4,
);
...
assert.equal(
  (body(simd, "v256AllTrueI8").match(/i8x16\.all_true/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v256AllTrueI32").match(/i32x4\.all_true/g) ?? []).length,
  2,
);
assert.doesNotMatch(body(simd, "v256AllTrueI64"), /i64x2\.all_true/);
assert.equal(
  (body(simd, "v512AllTrueI8").match(/i8x16\.all_true/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512AllTrueI32").match(/i32x4\.all_true/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512AllTrueI64").match(/i64x2\.all_true/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v256BitmaskI8").match(/i8x16\.bitmask/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v256BitmaskI32").match(/i32x4\.bitmask/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v256BitmaskI64").match(/i64x2\.bitmask/g) ?? []).length,
  2,
);
assert.equal(
  (body(simd, "v512BitmaskI8").match(/i8x16\.bitmask/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512BitmaskI32").match(/i32x4\.bitmask/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512BitmaskI64").match(/i64x2\.bitmask/g) ?? []).length,
  4,
);
assert.equal((body(simd, "v512ShlI16").match(/i16x8\.shl/g) ?? []).length, 4);
assert.equal((body(simd, "v512LtI16").match(/i16x8\.lt_s/g) ?? []).length, 4);
assert.equal(
  (body(simd, "v512AddSatI16").match(/i16x8\.add_sat_s/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512SubSatI16").match(/i16x8\.sub_sat_s/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512AvgrU8").match(/i8x16\.avgr_u/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512AvgrU16").match(/i16x8\.avgr_u/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512MulI8").match(/i16x8\.extmul_low_i8x16_u/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512MulI8").match(/i16x8\.extmul_high_i8x16_u/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512MulI8").match(/i8x16\.shuffle/g) ?? []).length,
  4,
);
...
assert.equal(
  (body(simd, "v512SplatI64").match(/i64x2\.splat/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v512SplatI64").match(/v128\.store/g) ?? []).length,
  4,
);
...
assert.equal(
  (body(simd, "v512ReplaceLane").match(/v128\.load/g) ?? []).length,
  4,
);
assert.equal(
  (body(simd, "v512ReplaceLane").match(/v128\.store/g) ?? []).length,
  4,
);
...
for (const mode of [swar, simd]) {
  ...
  assert.doesNotMatch(
    lane,
    /v128\.(?:load|store)|i64\.(?:shr|shl)/,
    "v512 lane extraction must remain one direct scalar load",
  );
}
assert.equal(
  (body(simd, "v128MulI8").match(/i16x8\.extmul_low_i8x16_u/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128MulI8").match(/i16x8\.extmul_high_i8x16_u/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128MulI8").match(/i8x16\.shuffle/g) ?? []).length,
  1,
);
...
assert.equal(
  (body(simd, "v128AddSatI8").match(/i8x16\.add_sat_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128AddSatI16").match(/i16x8\.add_sat_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128SubSatI8").match(/i8x16\.sub_sat_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128SubSatI16").match(/i16x8\.sub_sat_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128AvgrU8").match(/i8x16\.avgr_u/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128AvgrU16").match(/i16x8\.avgr_u/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128PopcntI8").match(/i8x16\.popcnt/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128DotI16").match(/i32x4\.dot_i16x8_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128Swizzle").match(/i8x16\.swizzle/g) ?? []).length,
  1,
);
...
assert.equal(
  (body(simd, "v128NearestF32").match(/f32x4\.nearest/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128NearestF64").match(/f64x2\.nearest/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128ConvertI32").match(/f32x4\.convert_i32x4_s/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128ConvertLowI32").match(/f64x2\.convert_low_i32x4_s/g) ?? [])
    .length,
  0,
);
assert.equal(
  (body(simd, "v128TruncSatI32").match(/i32x4\.trunc_sat_f32x4_s/g) ?? [])
    .length,
  1,
);
assert.equal(
  (body(simd, "v128NarrowI16").match(/i8x16\.narrow_i16x8_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128NarrowI32").match(/i16x8\.narrow_i32x4_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128ExtendLowI8").match(/i16x8\.extend_low_i8x16_s/g) ?? [])
    .length,
  1,
);
assert.equal(
  (body(simd, "v128ExtaddI8").match(/i16x8\.extadd_pairwise_i8x16_s/g) ?? [])
    .length,
  1,
);
assert.equal(
  (body(simd, "v128ExtmulI8").match(/i16x8\.extmul_low_i8x16_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128Q15Mulr").match(/i16x8\.q15mulr_sat_s/g) ?? []).length,
  1,
);
for (const name of [
  "v128ExtendLowI16",
  ...
  assert.doesNotMatch(
    body(simd, name),
    /(?:i8x16|i16x8|i32x4|i64x2|f32x4|f64x2)\.(?:extend|extmul|extadd|trunc_sat|demote|promote)/,
    `${name} reintroduced a scalar/SIMD crossing`,
  );
}
...
assert.equal(
  (body(simd, "v128AnyTrue").match(/v128\.any_true/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128AllTrueI8").match(/i8x16\.all_true/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128AllTrueI32").match(/i32x4\.all_true/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128AllTrueI64").match(/i64x2\.all_true/g) ?? []).length,
  0,
);
assert.equal(
  (body(simd, "v128BitmaskI32").match(/i32x4\.bitmask/g) ?? []).length,
  1,
);
for (const name of [
  "v128MinI32",
  ...
  assert.equal(
    (
      body(simd, name).match(/(?:i32x4|i64x2)\.(?:min|max|abs|neg|shl|shr)/g) ??
      []
    ).length,
    0,
    `${name} reintroduced a scalar/SIMD crossing`,
...
for (const name of [
  ...
  assert.equal(
    (body(simd, name).match(/(?:i8x16|i16x8)\.(?:neg|abs|shl|shr_[su])/g) ?? [])
      .length,
    0,
    `${name} reintroduced a scalar/SIMD crossing`,
  );
...
for (const name of [
  ...
  assert.equal(
    (body(simd, name).match(/(?:i8x16|i16x8|i32x4|i64x2)\.(?:add|sub)/g) ?? [])
      .length,
    0,
    `${name} reintroduced a scalar/SIMD crossing`,
  );
...
assert.equal(
  (body(simd, "v128rMulI8").match(/i8x16\.shuffle/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rAddSatI16").match(/i16x8\.add_sat_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rBitmaskI8").match(/i8x16\.bitmask/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rSwizzle").match(/i8x16\.swizzle/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rNarrowI16").match(/i8x16\.narrow_i16x8_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rLoadExtI8").match(/v128\.load8x8_s/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rLoadZeroI32").match(/v128\.load32_zero/g) ?? []).length,
  1,
);
assert.equal(
  (body(simd, "v128rLoadSplatI8").match(/v128\.load8_splat/g) ?? []).length,
  1,
);
assert.equal((body(simd, "v128rAbsI16").match(/i16x8\.abs/g) ?? []).length, 1);
assert.equal(
  (body(simd, "v128rAllTrueI32").match(/i32x4\.all_true/g) ?? []).length,
  1,
);
for (const [name, opcode] of [
  ...
  assert.doesNotMatch(
    body(simd, name),
    opcode,
    `${name} reintroduced a measured scalar/SIMD crossing`,
  );
}
for (const mode of [swar, simd]) {
