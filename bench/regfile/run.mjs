...
const aLo = 0xfedcba9876543210n,
  aHi = 0x0123456789abcdefn;
const bLo = 0x8899aabbccddeeffn,
  bHi = 0x1020304050607080n;
...
for (const [g, variants] of Object.entries(groups)) {
  ...
  for (const r of res) {
    ...
    console.log(
      `  ${r.name.padEnd(11)} best=${r.bestMs.toFixed(1)}ms med=${r.medMs.toFixed(1)}ms  ${r.mops.toFixed(0).padStart(5)} Mops/s  (${rel}% of fastest)`,
    );
  }
  console.log("");
}
