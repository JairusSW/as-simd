...
console.log(
  `iters=${ITERS.toLocaleString()} rounds=${ROUNDS} best-of (Mops/s)\n`,
);
console.log(
  "op".padEnd(13),
  "SWAR".padStart(8),
...
for (const op of ops) {
  ...
  const win =
    si > sw
      ? `SIMD +${((si / sw - 1) * 100).toFixed(0)}%`
      : `SWAR +${((sw / si - 1) * 100).toFixed(0)}%`;
  console.log(
    op.padEnd(13),
    sw.toFixed(0).padStart(8),
    si.toFixed(0).padStart(8),
    "  " + win,
  );
}
