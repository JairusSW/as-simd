...
const MD_OUT = path.join(CHARTS_DIR, "chart-i16x8-swar-v-i16x8-simd.md");
const SVG_OUT = path.join(CHARTS_DIR, "chart-i16x8-swar-v-i16x8-simd.svg");
const TITLE = "i16x8 SWAR vs SIMD";
const SUBTITLE = chartSubtitle(ROOT);

const SUITE_SWAR = "i16x8";
const SUITE_SIMD = "i16x8";
const ORDER = [
  ...
  "narrow-i32x4-s", "narrow-i32x4-u", "extend-low-i8x16-s", "extend-low-i8x16-u",
  "extend-high-i8x16-s", "extend-high-i8x16-u", "extadd-pairwise-i8x16-s", "extadd-pairwise-i8x16-u",
  "q15mulr-sat-s", "extmul-low-i8x16-s", "extmul-low-i8x16-u", "extmul-high-i8x16-s", "extmul-high-i8x16-u",
  "shuffle", "relaxed-laneselect", "relaxed-q15mulr-s", "relaxed-dot-i8x16-i7x16-s",
];

type Variant = { key: string; color: string; runtime: "v8" | "wavm"; mode: "swar" | "simd"; suite: string };
const VARIANTS: Variant[] = [
  { key: "i16x8-swar-v8", color: "rgb(99,102,241)", runtime: "v8", mode: "swar", suite: SUITE_SWAR },
  { key: "i16x8-swar-wavm", color: "rgb(34,197,94)", runtime: "wavm", mode: "swar", suite: SUITE_SWAR },
  { key: "i16x8-simd-v8", color: "rgb(234,220,90)", runtime: "v8", mode: "simd", suite: SUITE_SIMD },
  { key: "i16x8-simd-wavm", color: "rgb(239,68,68)", runtime: "wavm", mode: "simd", suite: SUITE_SIMD },
];
...
for (const v of VARIANTS) {
  for (const k of Object.keys(loaded[v.key])) suffixes.add(k);
}
