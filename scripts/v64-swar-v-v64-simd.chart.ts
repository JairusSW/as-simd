...
const MD_OUT = path.join(CHARTS_DIR, "chart-v64-swar-v-v64-simd.md");
const SVG_OUT = path.join(CHARTS_DIR, "chart-v64-swar-v-v64-simd.svg");
const TITLE = "v64 SWAR vs SIMD";
const SUBTITLE = chartSubtitle(ROOT);

const SUITE_SWAR = "v64";
const SUITE_SIMD = "v64";
...
const VARIANTS: Variant[] = [
  ...
  { key: "v64-swar-llvm", color: "rgb(34,197,94)", runtime: "wavm", mode: "swar", suite: SUITE_SWAR },
  { key: "v64-simd-v8", color: "rgb(234,220,90)", runtime: "v8", mode: "simd", suite: SUITE_SIMD },
  { key: "v64-simd-llvm", color: "rgb(239,68,68)", runtime: "wavm", mode: "simd", suite: SUITE_SIMD },
];
...
if (!rows.length) {
  console.error(`No overlapping benchmark methods found for ${SUITE_SWAR} (swar) and ${SUITE_SIMD} (simd) in build/logs/as`);
  process.exit(1);
}
