...
type Variant = { key: string; label: string; color: string; mode: "swar" | "simd"; runtime: "v8" | "wavm" };

const VARIANTS: Variant[] = [
  { key: "swar-v8", label: "swar v8", color: "rgb(99,102,241)", mode: "swar", runtime: "v8" },
  { key: "swar-wavm", label: "swar wavm", color: "rgb(34,197,94)", mode: "swar", runtime: "wavm" },
  { key: "simd-v8", label: "simd v8", color: "rgb(234,220,90)", mode: "simd", runtime: "v8" },
  { key: "simd-wavm", label: "simd wavm", color: "rgb(239,68,68)", mode: "simd", runtime: "wavm" },
];
...
function loadSuite(mode: string, suite: string, runtime: "v8" | "wavm"): Record<string, BenchResult> {
  ...
  for (const file of fs.readdirSync(dir)) {
    ...
    if (!file.startsWith(withRuntime) || !file.endsWith(`.${runtime}.json`)) continue;
...
md.push("");
md.push("Sources:");
md.push("- `build/logs/as/swar/" + SUITE + ".*.v8.json`");
md.push("- `build/logs/as/simd/" + SUITE + ".*.v8.json`");
md.push("- `build/logs/as/swar/" + SUITE + ".*.wavm.json`");
md.push("- `build/logs/as/simd/" + SUITE + ".*.wavm.json`");
md.push("");
