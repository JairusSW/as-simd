...
const SUITE_A = "i32x2";
const SUITE_B = "i32x4";
const MD_OUT = path.join(CHARTS_DIR, "chart-i32x2-swar-v-i32x4-simd.md");
const SVG_OUT = path.join(CHARTS_DIR, "chart-i32x2-swar-v-i32x4-simd.svg");
const TITLE = "i32x2 SWAR vs i32x4 SIMD";
...
const VARIANTS: Variant[] = [
  { key: "i32x2-swar-v8", color: "rgb(99,102,241)", runtime: "v8", mode: "swar", suite: SUITE_A },
  { key: "i32x2-swar-llvm", color: "rgb(34,197,94)", runtime: "llvm", mode: "swar", suite: SUITE_A },
  { key: "i32x4-simd-v8", color: "rgb(234,220,90)", runtime: "v8", mode: "simd", suite: SUITE_B },
  { key: "i32x4-simd-llvm", color: "rgb(239,68,68)", runtime: "llvm", mode: "simd", suite: SUITE_B },
];
...
const rows = DECL_ORDER.map((op) => {
  ...
  const avg = values.reduce((s, x) => s + x.ops, 0) / count;
  const dV8 = values[0].ok && values[2].ok ? pctDelta(values[0].ops, values[2].ops) : null;
  return { op, values, avg, dV8 };
}).filter((x): x is NonNullable<typeof x> => x != null).sort((a, b) => b.avg - a.avg);
