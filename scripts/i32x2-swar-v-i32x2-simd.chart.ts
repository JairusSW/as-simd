...
const SUITE_A = "i32x2";
const SUITE_B = "i32x2";
const MD_OUT = path.join(CHARTS_DIR, "chart-i32x2-swar-v-i32x2-simd.md");
const SVG_OUT = path.join(CHARTS_DIR, "chart-i32x2-swar-v-i32x2-simd.svg");
const TITLE = "i32x2 SWAR vs SIMD";
...
const VARIANTS: Variant[] = [
  ...
  { key: "i32x2-swar-llvm", color: "rgb(34,197,94)", runtime: "wavm", mode: "swar", suite: SUITE_A },
  { key: "i32x2-simd-v8", color: "rgb(234,220,90)", runtime: "v8", mode: "simd", suite: SUITE_B },
  { key: "i32x2-simd-llvm", color: "rgb(239,68,68)", runtime: "wavm", mode: "simd", suite: SUITE_B },
];
...
const rows = orderedOps.map((op) => {
  ...
  const ai = DECL_ORDER.indexOf(a.op);
  const bi = DECL_ORDER.indexOf(b.op);
