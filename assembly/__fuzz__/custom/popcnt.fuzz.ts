...
@inline function check(a: u64): bool {
  const expected = i8x8_scalar.popcnt(a);
  const lib = i8x8.popcnt(a);
  const current = popcnt_current(a);
  const split32 = popcnt_split32(a);
  const intrinsic = popcnt_intrinsic_lanes(a);
  if (
    lib != expected ||
    current != expected ||
    split32 != expected ||
    ...
    expect<u64>(intrinsic).toBe(expected);
    return false;
  }
  return true;
}
fuzz("i8x8.popcnt candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
  const cases: u64[] = [
    0, 1, 0xffffffffffffffff, 0x0102040810204080, 0xfedcba9876543210,
  ];
