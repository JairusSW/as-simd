...
@inline function check(a: u64, b: u64): bool {
  const expected = i8x8_scalar.max_u(a, b);
  const lib = i8x8.max_u(a, b);
  const current = max_u_current(a, b);
  const split16 = max_u_split16(a, b);
  const dual = max_u_min_dual(a, b);
  if (
    lib != expected ||
    current != expected ||
    split16 != expected ||
    ...
    expect<u64>(dual).toBe(expected);
    return false;
  }
  return true;
}
fuzz("i8x8.max_u candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
  ...
  if (!check(0x7f7f7f7f7f7f7f7f, 0x8080808080808080)) return false;
