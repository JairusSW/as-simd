...
@inline function check(a: u64, b: u64): bool {
  const expected = i8x8_scalar.sub_sat_s(a, b);
  const lib = i8x8.sub_sat_s(a, b);
  const current = sub_sat_s_current(a, b);
  const split16 = sub_sat_s_split16(a, b);
  const split32 = sub_sat_s_split32(a, b);
  if (
    lib != expected ||
    current != expected ||
    split16 != expected ||
...
fuzz("i8x8.sub_sat_s candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
  const cases: u64[] = [
    0, 0xffffffffffffffff, 0x8080808080808080, 0x7f7f7f7f7f7f7f7f,
