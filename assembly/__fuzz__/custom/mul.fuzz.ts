...
// @ts-expect-error: decorator
@inline function mul_current(a: u64, b: u64): u64 {
  return (
    (((a & 0xff) * (b & 0xff)) & 0xff) |
...
@inline function check(a: u64, b: u64): bool {
  const expected = i8x8_scalar.mul(a, b);
  const lib = i8x8.mul(a, b);
  const current = mul_current(a, b);
  const split32 = mul_split32(a, b);
  const split16 = mul_split16(a, b);
  if (
    lib != expected ||
    current != expected ||
    split32 != expected ||
...
fuzz("i8x8.mul candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
  const cases: u64[] = [
    0, 1, 0xffffffffffffffff, 0xfedcba9876543210, 0x7766554433221100,
  ];
