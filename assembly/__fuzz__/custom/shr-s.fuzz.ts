...
@inline function check(a: u64, s: i32): bool {
  ...
  if (lib != expected || current != expected || sw != expected || split32 != expected) {
...
fuzz("i8x8.shr_s candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
  const cases: u64[] = [0, 0xffffffffffffffff, 0x8080808080808080, 0x0102040810204080, 0xfedcba9876543210];
  for (let c = 0; c < cases.length; c++) for (let s = 0; s < 16; s++) if (!check(cases[c], s)) return false;
  for (let i = 0; i < 64; i++) if (!check(nextU64(), <i32>nextU32())) return false;
  return true;
