...
@inline function check(a: u64, b: u64): bool {
  const expected = i8x8_scalar.min_u(a, b);
  const lib = i8x8.min_u(a, b);
  const current = min_u_current(a, b);
  const split32 = min_u_split32(a, b);
  const split16 = min_u_split16(a, b);
  if (
    lib != expected ||
    current != expected ||
    split32 != expected ||
...
fuzz("i8x8.min_u candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
