...
@inline function check(a: u64, b: u64): bool {
  ...
  if (
    ...
    current != expected ||
    neInverse != expected ||
    neXorHighbit != expected ||
    currentSplit32 != expected ||
    currentSplit16 != expected ||
    splitNonzero != expected
  ) {
...
fuzz("i8x8.eq candidates", (seedValue: i32): bool => {
  state = <u64>seedValue;
  const cases: u64[] = [
    0, 1, 0xffffffffffffffff, 0x0001000100010001, 0x0100010001000100,
    0x8080808080808080, 0x7f7f7f7f7f7f7f7f, 0xfedcba9876543210,
    0xfedcba8876543200,
  ];
