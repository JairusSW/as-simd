...
fuzz("i32x2 scalar reference parity", (words: u32[]): bool => {
  const a = u64At(words, 0);
  const b = u64At(words, 2);
  const m = u64At(words, 4);
  const idx = <u8>(unchecked(words[6]) & 1);
  const shift = <i32>(unchecked(words[7]) & 63);
  const laneVal = <i32>unchecked(words[8]);
  const l0 = <u8>(unchecked(words[9]) & 3);
  const l1 = <u8>(unchecked(words[10]) & 3);
  checkId = 1;
  ...
  if (!check64(i32x2.min_s(a, b), i32x2_scalar.min_s(a, b))) return false;
  ...
  if (!check64(i32x2.lt_s(a, b), i32x2_scalar.lt_s(a, b))) return false;
  if (!check64(i32x2.lt_u(a, b), i32x2_scalar.lt_u(a, b))) return false;
  if (!check64(i32x2.le_s(a, b), i32x2_scalar.le_s(a, b))) return false;
  if (!check64(i32x2.le_u(a, b), i32x2_scalar.le_u(a, b))) return false;
  if (!check64(i32x2.gt_s(a, b), i32x2_scalar.gt_s(a, b))) return false;
