...
// @ts-expect-error: decorator
@inline function nextLen16(): i32 { return <i32>(nextA64() & 31) - 8; }

bench("i8x16_swar.splat", () => { blackbox(i8x16_swar.splat(nextI8())); }, OPS, 16); dumpToFile("i8x16-swar", "splat");
bench("i8x16_swar.load", () => { blackbox(load<v128>(nextPtr16())); }, OPS, 16); dumpToFile("i8x16-swar", "load");
bench("i8x16_swar.store", () => { store<v128>(nextPtr16(), nextVecA()); blackbox(load<u64>(IO_PTR)); }, OPS, 16); dumpToFile("i8x16-swar", "store");
bench("i8x16_swar.loadPartial", () => { blackbox(i8x16_swar.loadPartial(nextPtr16(), nextLen16(), 0, 1, nextI8())); }, OPS, 16); dumpToFile("i8x16-swar", "load-partial");
bench("i8x16_swar.storePartial", () => { i8x16_swar.storePartial(nextPtr16(), nextVecA(), nextLen16(), 0, 1); blackbox(load<u64>(IO_PTR)); }, OPS, 16); dumpToFile("i8x16-swar", "store-partial");
