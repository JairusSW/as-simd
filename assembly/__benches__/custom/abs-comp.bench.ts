...
// @ts-expect-error: decorator
@inline function abs_lib(a: u64): u64 {
  return i8x8.abs(a);
}
// @ts-expect-error: decorator
...
bench(
  "abs.lib",
  () => {
    blackbox(abs_lib(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "lib");
bench(
  "abs.current",
  () => {
    blackbox(abs_current(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "current");
bench(
  "abs.neg-select",
  () => {
    blackbox(abs_neg_select(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "neg-select");
bench(
  "abs.neg-add-select",
  () => {
    blackbox(abs_neg_add_select(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "neg-add-select");
bench(
  "abs.xor-add",
  () => {
    blackbox(abs_xor_add(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "xor-add");
bench(
  "abs.xor-sub",
  () => {
    blackbox(abs_xor_sub(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "xor-sub");
bench(
  "abs.split32",
  () => {
    blackbox(abs_split32(blackbox(abs_a)));
  },
  OPS,
  8,
);
dumpToFile("abs-comp", "split32");
