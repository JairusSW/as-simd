...
@inline function mul_split32(a: u64, b: u64): u64 {
  const lo =
    ...
    (((((a >> 16) & 0xffff) * ((b >> 16) & 0xffff)) & 0xffff) << 16);
  const hi =
    ((((a >> 32) & 0xffff) * ((b >> 32) & 0xffff)) & 0xffff) |
    (((((a >> 48) & 0xffff) * ((b >> 48) & 0xffff)) & 0xffff) << 16);
...
dumpToFile("i16x4-mul-comp", "lib");
bench(
  "i16x4-mul.pack4",
  () => {
    blackbox(mul_pack4(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("i16x4-mul-comp", "pack4");
bench(
  "i16x4-mul.locals",
  () => {
    blackbox(mul_locals(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("i16x4-mul-comp", "locals");
bench(
  "i16x4-mul.split32",
  () => {
...
dumpToFile("i16x4-mul-comp", "split32");
