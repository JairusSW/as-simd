...
@inline function sat_s_select(x: i32): u64 {
  return (
    select<u64>(127, select<u64>(-128, x as u64, x < -128), x > 127) & 0xff
  );
}
...
bench(
  ...
  () => {
    blackbox(
      narrow_s_lib(blackbox(aLo), blackbox(aHi), blackbox(bLo), blackbox(bHi)),
    );
    blackbox(hi_sink);
  },
  OPS,
...
bench(
  ...
  () => {
    blackbox(
      narrow_u_lib(blackbox(aLo), blackbox(aHi), blackbox(bLo), blackbox(bHi)),
    );
    blackbox(hi_sink);
  },
  OPS,
