...
@inline function min_u_lib(a: u64, b: u64): u64 {
  return i8x8.min_u(a, b);
}
// @ts-expect-error: decorator
@inline function min_u_current(a: u64, b: u64): u64 {
  const mask = lt_mask_u_current(a, b);
  return b ^ ((a ^ b) & mask);
}
// @ts-expect-error: decorator
@inline function lt_mask_u_split32(a: u64, b: u64): u64 {
  const alo = a as u32,
    blo = b as u32;
...
// @ts-expect-error: decorator
@inline function min_u_split32(a: u64, b: u64): u64 {
  const mask = lt_mask_u_split32(a, b);
  return b ^ ((a ^ b) & mask);
}
// @ts-expect-error: decorator
@inline function min_u_split16(a: u64, b: u64): u64 {
  const dlo =
...
dumpToFile("min-u-comp", "lib");
bench(
  "min-u.current",
  () => {
    blackbox(min_u_current(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("min-u-comp", "current");
bench(
  "min-u.split32",
  () => {
    blackbox(min_u_split32(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("min-u-comp", "split32");
bench(
  "min-u.split16",
  () => {
    blackbox(min_u_split16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("min-u-comp", "split16");
