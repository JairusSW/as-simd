...
bench(
  "bitmask-lane.lib",
  () => {
    blackbox(i8x8.bitmask_lane(blackbox(a)));
  },
  OPS,
  8,
);
dumpToFile("bitmask-lane-comp", "lib");
bench(
  "bitmask-lane.current",
  () => {
    blackbox(bitmask_lane_current(blackbox(a)));
  },
  OPS,
  8,
);
dumpToFile("bitmask-lane-comp", "current");
bench(
  "bitmask-lane.from-ne",
  () => {
    blackbox(bitmask_lane_from_ne(blackbox(a)));
  },
  OPS,
  8,
);
dumpToFile("bitmask-lane-comp", "from-ne");
bench(
  "bitmask-lane.split32",
  () => {
    blackbox(bitmask_lane_split32(blackbox(a)));
  },
  OPS,
  8,
);
dumpToFile("bitmask-lane-comp", "split32");
