...
bench(
  "add-sat-s.lib",
  () => {
    blackbox(i8x8.add_sat_s(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("add-sat-s-comp", "lib");
bench(
  "add-sat-s.current",
  () => {
    blackbox(add_sat_s_current(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("add-sat-s-comp", "current");
bench(
  "add-sat-s.split16",
  () => {
    blackbox(add_sat_s_split16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("add-sat-s-comp", "split16");
bench(
  "add-sat-s.split32",
  () => {
    blackbox(add_sat_s_split32(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("add-sat-s-comp", "split32");
