...
bench(
  "sub-sat-u.lib",
  () => {
    blackbox(i8x8.sub_sat_u(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-u-comp", "lib");
bench(
  "sub-sat-u.current",
  () => {
    blackbox(sub_sat_u_current(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-u-comp", "current");
bench(
  "sub-sat-u.split16",
  () => {
    blackbox(sub_sat_u_split16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-u-comp", "split16");
bench(
  "sub-sat-u.min",
  () => {
    blackbox(sub_sat_u_min(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-u-comp", "min");
