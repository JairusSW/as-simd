...
@inline function avgr_lib(a: u64, b: u64): u64 {
  return i8x8.avgr_u(a, b);
}
// @ts-expect-error: decorator
...
bench(
  "avgr-u.lib",
  () => {
    blackbox(avgr_lib(blackbox(avgr_a), blackbox(avgr_b)));
  },
  OPS,
  8,
);
dumpToFile("avgr-u-comp", "lib");
bench(
  "avgr-u.current",
  () => {
    blackbox(avgr_current(blackbox(avgr_a), blackbox(avgr_b)));
  },
  OPS,
  8,
);
dumpToFile("avgr-u-comp", "current");
bench(
  "avgr-u.current-const",
  () => {
    blackbox(avgr_current_const(blackbox(avgr_a), blackbox(avgr_b)));
  },
  OPS,
  8,
);
dumpToFile("avgr-u-comp", "current-const");
bench(
  "avgr-u.or-xor-masked",
  () => {
    blackbox(avgr_or_xor_masked(blackbox(avgr_a), blackbox(avgr_b)));
  },
  OPS,
  8,
);
dumpToFile("avgr-u-comp", "or-xor-masked");
bench(
  "avgr-u.split32",
  () => {
    blackbox(avgr_split32(blackbox(avgr_a), blackbox(avgr_b)));
  },
  OPS,
  8,
);
dumpToFile("avgr-u-comp", "split32");
bench(
  "avgr-u.split16",
  () => {
    blackbox(avgr_split16(blackbox(avgr_a), blackbox(avgr_b)));
  },
  OPS,
  8,
);
dumpToFile("avgr-u-comp", "split16");
