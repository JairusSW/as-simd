...
dumpToFile("max-s-comp", "lib");
bench(
  "max-s.current",
  () => {
    blackbox(max_s_current(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("max-s-comp", "current");
bench(
  "max-s.split16",
  () => {
    blackbox(max_s_split16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("max-s-comp", "split16");
bench(
  "max-s.dual16",
  () => {
    blackbox(max_s_dual16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("max-s-comp", "dual16");
