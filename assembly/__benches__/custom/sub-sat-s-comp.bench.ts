...
@inline function sub_sat_s_lib(a: u64, b: u64): u64 {
  return i8x8.sub_sat_s(a, b);
}
// @ts-expect-error: decorator
@inline function sub_sat_s_current(a: u64, b: u64): u64 {
  const diff =
...
// @ts-expect-error: decorator
@inline function sub_sat_s_split16(a: u64, b: u64): u64 {
  const dlo =
...
// @ts-expect-error: decorator
@inline function sub_sat_s_split32(a: u64, b: u64): u64 {
  const alo = a as u32,
    blo = b as u32;
  ...
  const diff = (dlo as u64) | ((dhi as u64) << 32);
  const overflow = ((a ^ b) & (a ^ diff) & 0x8080808080808080) >> 7;
  const mask = overflow * 0xff;
  const limit = (((a & 0x8080808080808080) >> 7) * 0xff) ^ 0x7f7f7f7f7f7f7f7f;
  return (diff & ~mask) | (limit & mask);
...
bench(
  "sub-sat-s.lib",
  () => {
    blackbox(sub_sat_s_lib(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-s-comp", "lib");
bench(
  "sub-sat-s.current",
  () => {
    blackbox(sub_sat_s_current(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-s-comp", "current");
bench(
  "sub-sat-s.split16",
  () => {
    blackbox(sub_sat_s_split16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-s-comp", "split16");
bench(
  "sub-sat-s.split32",
  () => {
    blackbox(sub_sat_s_split32(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("sub-sat-s-comp", "split32");
