...
@inline function mul_lib(a: u64, b: u64): u64 {
  return i8x8.mul(a, b);
}
// @ts-expect-error: decorator
...
bench(
  "mul.lib",
  () => {
    blackbox(mul_lib(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("mul-comp", "lib");
bench(
  "mul.shift64",
  () => {
    blackbox(mul_shift64(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("mul-comp", "shift64");
bench(
  "mul.split32",
  () => {
    blackbox(mul_split32(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("mul-comp", "split32");
bench(
  "mul.split16",
  () => {
    blackbox(mul_split16(blackbox(a), blackbox(b)));
  },
  OPS,
  8,
);
dumpToFile("mul-comp", "split16");
