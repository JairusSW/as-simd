...
describe("32 through 512 bit vectors", () => {
  test("allocation-lean wide values preserve immutable semantics", () => {
    const a256 = v256.splat<i16>(30000),
      b256 = v256.splat<i16>(3000);
    ...
    const a512 = v512.splat<i8>(12),
      b512 = v512.splat<i8>(-3);
  ...
  test("public wide values preserve full-width lanes, masks, and shuffles", () => {
    ...
    let a256 = v256.splat<i64>(0),
      b256 = v256.splat<i64>(0);
    ...
    let a512 = v512.splat<i64>(0),
      b512 = v512.splat<i64>(0);
  ...
  test("wide lane namespaces widen and narrow across the full vector", () => {
    ...
    for (let i: u8 = 0; i < 32; i++)
      bytes256 = i8x32.replace_lane(bytes256, i, i as i8);
    ...
    expect<i32>(i8x32.extract_lane_u(i8x32.swizzle(bytes256, mask256), 0)).toBe(
      31,
    );

    let bytes512 = i8x64.splat(0);
    for (let i: u8 = 0; i < 64; i++)
      bytes512 = i8x64.replace_lane(bytes512, i, i as i8);
    ...
    expect<i32>(i8x64.extract_lane_u(i8x64.swizzle(bytes512, mask512), 0)).toBe(
      63,
    );
  });

  test("i64x4 and i64x8 agree with scalar edge-value semantics", () => {
    let a4 = i64x4.splat(0),
      b4 = i64x4.splat(0),
      m4 = i64x4.splat(0);
    ...
    const add4 = i64x4.add(a4, b4),
      sub4 = i64x4.sub(a4, b4),
      mul4 = i64x4.mul(a4, b4);
    const abs4 = i64x4.abs(a4),
      neg4 = i64x4.neg(a4);
    const shl4 = i64x4.shl(a4, 67),
      shrs4 = i64x4.shr_s(a4, 67),
      shru4 = i64x4.shr_u(a4, 67);
    const eq4 = i64x4.eq(a4, b4),
      ne4 = i64x4.ne(a4, b4);
    const lt4 = i64x4.lt_s(a4, b4),
      le4 = i64x4.le_s(a4, b4);
    const gt4 = i64x4.gt_s(a4, b4),
      ge4 = i64x4.ge_s(a4, b4);
    ...
    let x8 = i32x8.splat(0),
      y8 = i32x8.splat(0);
    for (let i: u8 = 0; i < 8; i++) {
      x8 = i32x8.replace_lane(
        x8,
        i,
        i == 0 ? i32.MIN_VALUE : (i as i32) * 0x20000001 - 7,
      );
      y8 = i32x8.replace_lane(
        y8,
        i,
        i == 7 ? i32.MAX_VALUE : ((11 - i) as i32) * 17,
      );
    }
    const exl4s = i64x4.extend_low_i32x8_s(x8),
      exl4u = i64x4.extend_low_i32x8_u(x8);
    const exh4s = i64x4.extend_high_i32x8_s(x8),
      exh4u = i64x4.extend_high_i32x8_u(x8);
    const ml4s = i64x4.extmul_low_i32x8_s(x8, y8),
      ml4u = i64x4.extmul_low_i32x8_u(x8, y8);
    const mh4s = i64x4.extmul_high_i32x8_s(x8, y8),
      mh4u = i64x4.extmul_high_i32x8_u(x8, y8);
    for (let i: u8 = 0; i < 4; i++) {
      const xl = i32x8.extract_lane(x8, i),
        xh = i32x8.extract_lane(x8, i + 4);
      const yl = i32x8.extract_lane(y8, i),
        yh = i32x8.extract_lane(y8, i + 4);
      ...
      expect<u64>(i64x4.extract_lane(ml4u, i) as u64).toBe(
        (xl as u32 as u64) * (yl as u32 as u64),
      );
      expect<i64>(i64x4.extract_lane(mh4s, i)).toBe((xh as i64) * yh);
      expect<u64>(i64x4.extract_lane(mh4u, i) as u64).toBe(
        (xh as u32 as u64) * (yh as u32 as u64),
      );
    }

    let a8 = i64x8.splat(0),
      b8 = i64x8.splat(0),
      m8 = i64x8.splat(0);
    for (let i: u8 = 0; i < 8; i++) {
      const a =
    ...
    const add8 = i64x8.add(a8, b8),
      sub8 = i64x8.sub(a8, b8),
      mul8 = i64x8.mul(a8, b8);
    const abs8 = i64x8.abs(a8),
      neg8 = i64x8.neg(a8);
    const shl8 = i64x8.shl(a8, 127),
      shrs8 = i64x8.shr_s(a8, 127),
      shru8 = i64x8.shr_u(a8, 127);
    const eq8 = i64x8.eq(a8, b8),
      ne8 = i64x8.ne(a8, b8),
      lt8 = i64x8.lt_s(a8, b8);
    const le8 = i64x8.le_s(a8, b8),
      gt8 = i64x8.gt_s(a8, b8),
      ge8 = i64x8.ge_s(a8, b8);
    const sel8 = i64x8.relaxed_laneselect(a8, b8, m8);
    for (let i: u8 = 0; i < 8; i++) {
      const a = i64x8.extract_lane(a8, i),
        b = i64x8.extract_lane(b8, i);
    ...
    for (let i: u8 = 0; i < 8; i++) {
      const expected =
        (i & 1) == 0
          ? i64x8.extract_lane(b8, 7 - i)
          : i64x8.extract_lane(a8, i - 1);
      expect<i64>(i64x8.extract_lane(shuffled8, i)).toBe(expected);
    }

    let x16 = i32x16.splat(0),
      y16 = i32x16.splat(0);
    for (let i: u8 = 0; i < 16; i++) {
      x16 = i32x16.replace_lane(
        x16,
        i,
        i == 0 ? i32.MIN_VALUE : (i as i32) * 0x10000001 - 9,
      );
      y16 = i32x16.replace_lane(
        y16,
        i,
        i == 15 ? i32.MAX_VALUE : ((13 - i) as i32) * 19,
      );
    }
    const exl8s = i64x8.extend_low_i32x16_s(x16),
      exl8u = i64x8.extend_low_i32x16_u(x16);
    const exh8s = i64x8.extend_high_i32x16_s(x16),
      exh8u = i64x8.extend_high_i32x16_u(x16);
    const ml8s = i64x8.extmul_low_i32x16_s(x16, y16),
      ml8u = i64x8.extmul_low_i32x16_u(x16, y16);
    const mh8s = i64x8.extmul_high_i32x16_s(x16, y16),
      mh8u = i64x8.extmul_high_i32x16_u(x16, y16);
    for (let i: u8 = 0; i < 8; i++) {
      const xl = i32x16.extract_lane(x16, i),
        xh = i32x16.extract_lane(x16, i + 8);
      const yl = i32x16.extract_lane(y16, i),
        yh = i32x16.extract_lane(y16, i + 8);
      ...
      expect<u64>(i64x8.extract_lane(ml8u, i) as u64).toBe(
        (xl as u32 as u64) * (yl as u32 as u64),
      );
      expect<i64>(i64x8.extract_lane(mh8s, i)).toBe((xh as i64) * yh);
      expect<u64>(i64x8.extract_lane(mh8u, i) as u64).toBe(
        (xh as u32 as u64) * (yh as u32 as u64),
      );
    }
  });
  ...
  test("dedicated wide I/O honors runtime byte offsets", () => {
    ...
    for (let i: u32 = 0; i < 4; i++)
      expect<u64>(load<u64>(wrf.addr(6) + ((i as usize) << 3))).toBe(
        memory[i + 1],
      );
    v256r.store(p, 6, 72);
    for (let i: u32 = 0; i < 4; i++)
      expect<u64>(memory[i + 9]).toBe(memory[i + 1]);

    v512r.load(7, p, 16);
    for (let i: u32 = 0; i < 8; i++)
      expect<u64>(load<u64>(wrf.addr(7) + ((i as usize) << 3))).toBe(
        memory[i + 2],
      );
    v512r.store(p, 7, 88);
    for (let i: u32 = 0; i < 8; i++)
      expect<u64>(memory[i + 11]).toBe(memory[i + 2]);
  });
});
