...
export namespace wrf {
  ...
  @inline export function addr(reg: u32): usize {
    return _base + ((reg as usize) << 6);
  }
  // @ts-expect-error: decorator
  @inline export function lo(reg: u32, chunk: u32 = 0): u64 {
    return load<u64>(addr(reg) + ((chunk as usize) << 4));
  }
  // @ts-expect-error: decorator
  @inline export function hi(reg: u32, chunk: u32 = 0): u64 {
    return load<u64>(addr(reg) + ((chunk as usize) << 4), 8);
  }
  // @ts-expect-error: decorator
  ...
  // @ts-expect-error: decorator
  @inline export function loadBits(
    reg: u32,
    ptr: usize,
    bits: u32,
    immOffset: usize = 0,
  ...
  // @ts-expect-error: decorator
  @inline export function storeBits(
    ptr: usize,
    reg: u32,
    bits: u32,
    immOffset: usize = 0,
