...
export namespace i8x16_swar {
  ...
  @inline export function splat(x: i8): readonly [u64, u64] {
    const p = (((x as u64) & 0xff) * 0x0101010101010101);
    return [p, p];
  }
  // @ts-expect-error: decorator
  @inline export function extract_lane_s(lo: u64, hi: u64, idx: u8): i8 {
    return ((select<u64>(hi, lo, idx < 8) >> ((idx & 7) << 3)) as u8) as i8;
  }
  // @ts-expect-error: decorator
  @inline export function extract_lane_u(lo: u64, hi: u64, idx: u8): u8 {
    return ((select<u64>(hi, lo, idx < 8) >> ((idx & 7) << 3)) as u8);
  }
  // @ts-expect-error: decorator
  @inline export function replace_lane(lo: u64, hi: u64, idx: u8, value: i8): readonly [u64, u64] {
    const i = idx & 15;
    const j = i & 7;
    ...
    if (i < 8) return [nextLo, hi];
    return [lo, nextHi];
  }
  // @ts-expect-error: decorator
  @inline export function loadPartial(ptr: usize, len: i32, immOffset: usize = 0, immAlign: usize = 1, fill: i8 = 0): readonly [u64, u64] {
    ...
    if (n <= 0) return [fillWord, fillWord];
    if (n >= 16) return [load<u64>(p), load<u64>(p + 8)];
    if (n < 8) {
      const bits = n << 3;
      const mask = ((1 as u64) << bits) - 1;
      return [(load<u64>(p) & mask) | (fillWord & ~mask), fillWord];
    }
    const bits = (n - 8) << 3;
  ...
  // @ts-expect-error: decorator
  @inline export function add(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function sub(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    return [
      ((aLo | 0x8080808080808080) - (bLo & ~0x8080808080808080)) ^ ((aLo ^ ~bLo) & 0x8080808080808080),
      ((aHi | 0x8080808080808080) - (bHi & ~0x8080808080808080)) ^ ((aHi ^ ~bHi) & 0x8080808080808080),
    ];
  ...
  // @ts-expect-error: decorator
  @inline export function mul(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function min_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    ...
    return [bLo ^ ((aLo ^ bLo) & mLo), bHi ^ ((aHi ^ bHi) & mHi)];
  }
  // @ts-expect-error: decorator
  @inline export function min_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
  ...
  // @ts-expect-error: decorator
  @inline export function max_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const axLo = aLo ^ 0x8080808080808080;
    const bxLo = bLo ^ 0x8080808080808080;
    const dLo = ((axLo | 0x8080808080808080) - (bxLo & 0x7f7f7f7f7f7f7f7f)) ^ ((axLo ^ ~bxLo) & 0x8080808080808080);
    const mLo = ((((~axLo & bxLo) | (~(axLo ^ bxLo) & dLo)) & 0x8080808080808080) >> 7) * 0xff;
    ...
    const dHi = ((axHi | 0x8080808080808080) - (bxHi & 0x7f7f7f7f7f7f7f7f)) ^ ((axHi ^ ~bxHi) & 0x8080808080808080);
    const mHi = ((((~axHi & bxHi) | (~(axHi ^ bxHi) & dHi)) & 0x8080808080808080) >> 7) * 0xff;
    return [aLo ^ ((aLo ^ bLo) & mLo), aHi ^ ((aHi ^ bHi) & mHi)];
  }
  // @ts-expect-error: decorator
  @inline export function max_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const dLo = ((aLo | 0x8080808080808080) - (bLo & 0x7f7f7f7f7f7f7f7f)) ^ ((aLo ^ ~bLo) & 0x8080808080808080);
    const mLo = ((((~aLo & bLo) | (~(aLo ^ bLo) & dLo)) & 0x8080808080808080) >> 7) * 0xff;
    const dHi = ((aHi | 0x8080808080808080) - (bHi & 0x7f7f7f7f7f7f7f7f)) ^ ((aHi ^ ~bHi) & 0x8080808080808080);
    const mHi = ((((~aHi & bHi) | (~(aHi ^ bHi) & dHi)) & 0x8080808080808080) >> 7) * 0xff;
  ...
  // @ts-expect-error: decorator
  @inline export function avgr_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function abs(aLo: u64, aHi: u64): readonly [u64, u64] {
    let mLo = ((aLo & 0x8080808080808080) >> 7) * 0xff;
    let xLo = aLo ^ mLo;
    let bLo = mLo & 0x0101010101010101;
    ...
    let mHi = ((aHi & 0x8080808080808080) >> 7) * 0xff;
    let xHi = aHi ^ mHi;
    let bHi = mHi & 0x0101010101010101;
    lo = (xHi & 0x0f0f0f0f0f0f0f0f) + (bHi & 0x0f0f0f0f0f0f0f0f);
    hi = (xHi & 0xf0f0f0f0f0f0f0f0) + (bHi & 0xf0f0f0f0f0f0f0f0) + (lo & 0x1010101010101010);
    return [outLo, (lo & 0x0f0f0f0f0f0f0f0f) | (hi & 0xf0f0f0f0f0f0f0f0)];
  }
  // @ts-expect-error: decorator
  @inline export function neg(aLo: u64, aHi: u64): readonly [u64, u64] {
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function add_sat_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    ...
    return [(sLo & ~mLo) | (lLo & mLo), (sHi & ~mHi) | (lHi & mHi)];
  }
  // @ts-expect-error: decorator
  @inline export function add_sat_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    ...
    return [outLo, (lo & 0x00ff00ff00ff00ff) | ((hi & 0x00ff00ff00ff00ff) << 8) | (lc - (lc >> 8)) | (hc * 0xff)];
  }
  // @ts-expect-error: decorator
  @inline export function sub_sat_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    ...
    const mHi = oHi * 0xff;
    const lHi = ((((aHi & 0x8080808080808080) >> 7) * 0xff) ^ 0x7f7f7f7f7f7f7f7f);
    return [(diffLo & ~mLo) | (lLo & mLo), (diffHi & ~mHi) | (lHi & mHi)];
  }
  // @ts-expect-error: decorator
  @inline export function sub_sat_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    ...
    const mHi = ((((~aHi & bHi) | (~(aHi ^ bHi) & dHi)) & 0x8080808080808080) >> 7) * 0xff;
    return [dLo & ~mLo, dHi & ~mHi];
  }
  // @ts-expect-error: decorator
  @inline export function shl(aLo: u64, aHi: u64, b: i32): readonly [u64, u64] {
    const s = b & 7;
    const mask = ((0xff >> s) as u64) * 0x0101010101010101;
    return [(aLo & mask) << s, (aHi & mask) << s];
  }
  // @ts-expect-error: decorator
  @inline export function shr_s(aLo: u64, aHi: u64, b: i32): readonly [u64, u64] {
    const s = b & 7;
    if (s == 0) return [aLo, aHi];
  ...
  // @ts-expect-error: decorator
  @inline export function shr_u(aLo: u64, aHi: u64, b: i32): readonly [u64, u64] {
    switch (b & 7) {
      case 0: return [aLo, aHi];
      case 1: return [(aLo >> 1) & 0x7f7f7f7f7f7f7f7f, (aHi >> 1) & 0x7f7f7f7f7f7f7f7f];
      case 2: return [(aLo >> 2) & 0x3f3f3f3f3f3f3f3f, (aHi >> 2) & 0x3f3f3f3f3f3f3f3f];
      case 3: return [(aLo >> 3) & 0x1f1f1f1f1f1f1f1f, (aHi >> 3) & 0x1f1f1f1f1f1f1f1f];
      case 4: return [(aLo >> 4) & 0x0f0f0f0f0f0f0f0f, (aHi >> 4) & 0x0f0f0f0f0f0f0f0f];
      case 5: return [(aLo >> 5) & 0x0707070707070707, (aHi >> 5) & 0x0707070707070707];
      case 6: return [(aLo >> 6) & 0x0303030303030303, (aHi >> 6) & 0x0303030303030303];
      default: return [(aLo >> 7) & 0x0101010101010101, (aHi >> 7) & 0x0101010101010101];
    }
  }
  // @ts-expect-error: decorator
  ...
  // @ts-expect-error: decorator
  @inline export function bitmask_lane(aLo: u64, aHi: u64): readonly [u64, u64] {
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function popcnt(aLo: u64, aHi: u64): readonly [u64, u64] {
  ...
  // @ts-expect-error: decorator
  @inline export function eq(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const xLo = aLo ^ bLo;
    ...
    return [~((mLo >> 7) * 0xff), ~((mHi >> 7) * 0xff)];
  }
  // @ts-expect-error: decorator
  @inline export function ne(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const xLo = aLo ^ bLo;
    const mLo = (((xLo & 0x7f7f7f7f7f7f7f7f) + 0x7f7f7f7f7f7f7f7f) | xLo) & 0x8080808080808080;
    const xHi = aHi ^ bHi;
    const mHi = (((xHi & 0x7f7f7f7f7f7f7f7f) + 0x7f7f7f7f7f7f7f7f) | xHi) & 0x8080808080808080;
  ...
  // @ts-expect-error: decorator
  @inline export function lt_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const p = ge_s(aLo, aHi, bLo, bHi);
    return [~p[0], ~p[1]];
  }
  // @ts-expect-error: decorator
  @inline export function lt_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const dLo = ((aLo | 0x8080808080808080) - (bLo & 0x7f7f7f7f7f7f7f7f)) ^ ((aLo ^ ~bLo) & 0x8080808080808080);
    const dHi = ((aHi | 0x8080808080808080) - (bHi & 0x7f7f7f7f7f7f7f7f)) ^ ((aHi ^ ~bHi) & 0x8080808080808080);
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function le_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    return ge_s(bLo, bHi, aLo, aHi);
  }
  // @ts-expect-error: decorator
  @inline export function le_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
  ...
  // @ts-expect-error: decorator
  @inline export function gt_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const bxLo = bLo ^ 0x8080808080808080;
    const axLo = aLo ^ 0x8080808080808080;
  ...
  // @ts-expect-error: decorator
  @inline export function gt_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const dLo = ((bLo | 0x8080808080808080) - (aLo & 0x7f7f7f7f7f7f7f7f)) ^ ((bLo ^ ~aLo) & 0x8080808080808080);
    const dHi = ((bHi | 0x8080808080808080) - (aHi & 0x7f7f7f7f7f7f7f7f)) ^ ((bHi ^ ~aHi) & 0x8080808080808080);
    return [
      ((((~bLo & aLo) | (~(bLo ^ aLo) & dLo)) & 0x8080808080808080) >> 7) * 0xff,
  ...
  // @ts-expect-error: decorator
  @inline export function ge_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const axLo = aLo ^ 0x8080808080808080;
    const bxLo = bLo ^ 0x8080808080808080;
    const dLo = ((axLo | 0x8080808080808080) - (bxLo & 0x7f7f7f7f7f7f7f7f)) ^ ((axLo ^ ~bxLo) & 0x8080808080808080);
    const axHi = aHi ^ 0x8080808080808080;
  ...
  // @ts-expect-error: decorator
  @inline export function ge_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    const dLo = ((aLo | 0x8080808080808080) - (bLo & 0x7f7f7f7f7f7f7f7f)) ^ ((aLo ^ ~bLo) & 0x8080808080808080);
    const dHi = ((aHi | 0x8080808080808080) - (bHi & 0x7f7f7f7f7f7f7f7f)) ^ ((aHi ^ ~bHi) & 0x8080808080808080);
    return [
      ~(((((~aLo & bLo) | (~(aLo ^ bLo) & dLo)) & 0x8080808080808080) >> 7) * 0xff),
  ...
  // @ts-expect-error: decorator
  @inline export function narrow_i16x8_s(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
    return [
  ...
  // @ts-expect-error: decorator
  @inline export function narrow_i16x8_u(aLo: u64, aHi: u64, bLo: u64, bHi: u64): readonly [u64, u64] {
  ...
  // @ts-expect-error: decorator
  @inline export function shuffle(aLo: u64, aHi: u64, bLo: u64, bHi: u64, l0: u8, l1: u8, l2: u8, l3: u8, l4: u8, l5: u8, l6: u8, l7: u8, l8: u8, l9: u8, l10: u8, l11: u8, l12: u8, l13: u8, l14: u8, l15: u8): readonly [u64, u64] {
  ...
  // @ts-expect-error: decorator
  @inline export function swizzle(aLo: u64, aHi: u64, sLo: u64, sHi: u64): readonly [u64, u64] {
  ...
  // @ts-expect-error: decorator
  @inline export function relaxed_swizzle(aLo: u64, aHi: u64, sLo: u64, sHi: u64): readonly [u64, u64] {
    const i0 = sLo as u8, i1 = (sLo >> 8) as u8, i2 = (sLo >> 16) as u8, i3 = (sLo >> 24) as u8;
    const i4 = (sLo >> 32) as u8, i5 = (sLo >> 40) as u8, i6 = (sLo >> 48) as u8, i7 = (sLo >> 56) as u8;
    const i8 = sHi as u8, i9 = (sHi >> 8) as u8, i10 = (sHi >> 16) as u8, i11 = (sHi >> 24) as u8;
    const i12 = (sHi >> 32) as u8, i13 = (sHi >> 40) as u8, i14 = (sHi >> 48) as u8, i15 = (sHi >> 56) as u8;
  ...
  // @ts-expect-error: decorator
  @inline export function relaxed_laneselect(aLo: u64, aHi: u64, bLo: u64, bHi: u64, mLo: u64, mHi: u64): readonly [u64, u64] {
    const ml = ((mLo & 0x8080808080808080) >> 7) * 0xff;
    const mh = ((mHi & 0x8080808080808080) >> 7) * 0xff;
    return [(bLo & ~ml) | (aLo & ml), (bHi & ~mh) | (aHi & mh)];
  }
}
