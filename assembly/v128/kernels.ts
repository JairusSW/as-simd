...
@inline function lane_load<T>(ptr: usize): T {
  return load<T>(ptr);
}


@inline function lane_store<T>(ptr: usize, value: T): void {
  store<T>(ptr, value);
}
...
export namespace v128_kernels {
  ...
  @inline function store_lo_hi(dst: u32, lo: u64): void {
    rf.set(dst, lo, v128_swar.take_hi());
  }

  // ---- construction / lanes ------------------------------------------------
  // @ts-expect-error: decorator
  @inline export function splat<T>(dst: u32, x: T): void {
    store_lo_hi(dst, v128_swar.splat<T>(x));
  }
  // @ts-expect-error: decorator
  ...
  @inline export function replace_lane<T>(
    ...
    const dp = rf.addr(dst),
      ap = rf.addr(a);
    if (dst != a) {
      if (ASC_FEATURE_SIMD) v128.store(dp, v128.load(ap));
      else {
        lane_store<u64>(dp, lane_load<u64>(ap));
        lane_store<u64>(dp + 8, lane_load<u64>(ap + 8));
      }
    }
    const lane = (idx as u32) % ((16 / sizeof<T>()) as u32);
  ...
  // @ts-expect-error: decorator
  @inline export function shuffle<T>(
    dst: u32,
    a: u32,
    b: u32,
    ...
    store_lo_hi(
      dst,
      v128_swar.shuffle<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b), lanes),
    );
  }
  // @ts-expect-error: decorator
  @inline export function swizzle(dst: u32, a: u32, s: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(
        rf.addr(dst),
        i8x16.swizzle(v128.load(rf.addr(a)), v128.load(rf.addr(s))),
      );
      return;
    }
  ...
  @inline export function load(
    ...
    store_lo_hi(dst, v128_swar.load(ptr, immOffset, immAlign));
  }
  // @ts-expect-error: decorator
  ...
  @inline export function store(
    ...
    v128_swar.store(ptr, rf.lo(a), rf.hi(a), immOffset, immAlign);
  }
  // @ts-expect-error: decorator
  @inline export function storePartial(
    ...
    v128_swar.storePartial(ptr, rf.lo(a), rf.hi(a), len, immOffset, immAlign);
  }
  // @ts-expect-error: decorator
  @inline export function load_ext<TFrom>(
    ...
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.load_ext<TFrom>(ptr + immOffset, 0, 1));
      return;
    }
  ...
  @inline export function load_zero<TFrom>(
    ...
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.load_zero<TFrom>(ptr + immOffset, 0, 1));
      return;
    }
  ...
  @inline export function load_splat<T>(
    ...
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.load_splat<T>(ptr + immOffset, 0, 1));
      return;
    }
  ...
  @inline export function add<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && isFloat<T>()) {
      v128.store(
        rf.addr(dst),
        v128.add<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function sub<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && isFloat<T>()) {
      v128.store(
        rf.addr(dst),
        v128.sub<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function mul<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      const av = v128.load(rf.addr(a)),
        bv = v128.load(rf.addr(b));
      if (sizeof<T>() == 1) {
        const lo = i16x8.extmul_low_i8x16_u(av, bv),
          hi = i16x8.extmul_high_i8x16_u(av, bv);
        v128.store(
          rf.addr(dst),
          i8x16.shuffle(
            lo,
  ...
  @inline export function div<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && isFloat<T>()) {
      v128.store(
        rf.addr(dst),
        v128.div<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function neg<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && isFloat<T>()) {
      v128.store(rf.addr(dst), v128.neg<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function abs<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      v128.store(rf.addr(dst), v128.abs<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function add_sat<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() <= 2) {
      v128.store(
        rf.addr(dst),
        v128.add_sat<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.add_sat<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function sub_sat<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() <= 2) {
      v128.store(
        rf.addr(dst),
        v128.sub_sat<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.sub_sat<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function avgr<T>(dst: u32, a: u32, b: u32): void {
    store_lo_hi(dst, v128_swar.avgr<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)));
  }
  // @ts-expect-error: decorator
  @inline export function min<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      ...
      if (sizeof<T>() == 1)
        v128.store(
          rf.addr(dst),
          isSigned<T>() ? v128.min<i8>(av, bv) : v128.min<u8>(av, bv),
        );
      else if (sizeof<T>() == 2)
        v128.store(
          rf.addr(dst),
          isSigned<T>() ? v128.min<i16>(av, bv) : v128.min<u16>(av, bv),
        );
      else if (sizeof<T>() == 4)
        v128.store(rf.addr(dst), v128.min<f32>(av, bv));
  ...
  @inline export function max<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      ...
      if (sizeof<T>() == 1)
        v128.store(
          rf.addr(dst),
          isSigned<T>() ? v128.max<i8>(av, bv) : v128.max<u8>(av, bv),
        );
      else if (sizeof<T>() == 2)
        v128.store(
          rf.addr(dst),
          isSigned<T>() ? v128.max<i16>(av, bv) : v128.max<u16>(av, bv),
        );
      else if (sizeof<T>() == 4)
        v128.store(rf.addr(dst), v128.max<f32>(av, bv));
  ...
  // @ts-expect-error: decorator
  @inline export function pmin<T>(dst: u32, a: u32, b: u32): void {
    min<T>(dst, a, b);
  }
  // @ts-expect-error: decorator
  @inline export function pmax<T>(dst: u32, a: u32, b: u32): void {
    max<T>(dst, a, b);
  }
  // @ts-expect-error: decorator
  @inline export function dot<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.dot<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function sqrt<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 4) {
      v128.store(rf.addr(dst), v128.sqrt<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function ceil<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 4) {
      v128.store(rf.addr(dst), v128.ceil<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function floor<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 4) {
      v128.store(rf.addr(dst), v128.floor<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function trunc<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 4) {
      v128.store(rf.addr(dst), v128.trunc<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function nearest<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 4) {
      v128.store(rf.addr(dst), v128.nearest<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function and(dst: u32, a: u32, b: u32): void {
    store_lo_hi(dst, v128_swar.and(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)));
  }
  // @ts-expect-error: decorator
  @inline export function or(dst: u32, a: u32, b: u32): void {
    store_lo_hi(dst, v128_swar.or(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)));
  }
  // @ts-expect-error: decorator
  @inline export function xor(dst: u32, a: u32, b: u32): void {
    store_lo_hi(dst, v128_swar.xor(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)));
  }
  // @ts-expect-error: decorator
  @inline export function andnot(dst: u32, a: u32, b: u32): void {
    store_lo_hi(dst, v128_swar.andnot(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)));
  }
  // @ts-expect-error: decorator
  @inline export function not(dst: u32, a: u32): void {
    store_lo_hi(dst, v128_swar.not(rf.lo(a), rf.hi(a)));
  }
  // @ts-expect-error: decorator
  @inline export function bitselect(dst: u32, v1: u32, v2: u32, m: u32): void {
    store_lo_hi(
      dst,
      v128_swar.bitselect(
        rf.lo(v1),
  ...
  // @ts-expect-error: decorator
  @inline export function popcnt<T>(dst: u32, a: u32): void {
    store_lo_hi(dst, v128_swar.popcnt<T>(rf.lo(a), rf.hi(a)));
  }

  // ---- shifts --------------------------------------------------------------
  // @ts-expect-error: decorator
  @inline export function shl<T>(dst: u32, a: u32, b: i32): void {
    store_lo_hi(dst, v128_swar.shl<T>(rf.lo(a), rf.hi(a), b));
  }
  // @ts-expect-error: decorator
  @inline export function shr<T>(dst: u32, a: u32, b: i32): void {
    store_lo_hi(dst, v128_swar.shr<T>(rf.lo(a), rf.hi(a), b));
  }
  ...
  @inline export function eq<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      v128.store(
        rf.addr(dst),
        v128.eq<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function ne<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      v128.store(
        rf.addr(dst),
        v128.ne<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function lt<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      v128.store(
        rf.addr(dst),
        v128.lt<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function le<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && (sizeof<T>() <= 2 || isFloat<T>())) {
      v128.store(
        rf.addr(dst),
        v128.le<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
  ...
  @inline export function any_true(a: u32): bool {
    return v128_swar.any_true(rf.lo(a), rf.hi(a));
  }
  // @ts-expect-error: decorator
  @inline export function all_true<T>(a: u32): bool {
    if (ASC_FEATURE_SIMD && sizeof<T>() <= 4)
      return v128.all_true<T>(v128.load(rf.addr(a)));
  ...
  @inline export function convert<TFrom>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.convert<TFrom>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function convert_low<TFrom>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.convert_low<TFrom>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function trunc_sat<TTo>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.trunc_sat<TTo>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function trunc_sat_zero<TTo>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.trunc_sat_zero<TTo>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function narrow<TFrom>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.narrow<TFrom>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.narrow<TFrom>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function extend_low<TFrom>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<TFrom>() == 1) {
      v128.store(rf.addr(dst), v128.extend_low<TFrom>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function extend_high<TFrom>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<TFrom>() == 1) {
      v128.store(rf.addr(dst), v128.extend_high<TFrom>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function extadd_pairwise<TFrom>(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<TFrom>() == 1) {
      v128.store(
        rf.addr(dst),
        v128.extadd_pairwise<TFrom>(v128.load(rf.addr(a))),
      );
      return;
    }
  ...
  @inline export function extmul_low<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 1) {
      v128.store(
        rf.addr(dst),
        v128.extmul_low<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.extmul_low<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function extmul_high<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD && sizeof<T>() == 1) {
      v128.store(
        rf.addr(dst),
        v128.extmul_high<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.extmul_high<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function demote_zero(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.demote_zero(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function promote_low(dst: u32, a: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(rf.addr(dst), v128.promote_low(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function q15mulr_sat<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.q15mulr_sat<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.q15mulr_sat<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  ...
  @inline export function relaxed_swizzle(dst: u32, a: u32, s: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.relaxed_swizzle(v128.load(rf.addr(a)), v128.load(rf.addr(s))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.relaxed_swizzle(rf.lo(a), rf.hi(a), rf.lo(s), rf.hi(s)),
    );
  }
  // @ts-expect-error: decorator
  ...
  @inline export function relaxed_laneselect<T>(
    ...
    store_lo_hi(
      dst,
      v128_swar.relaxed_laneselect<T>(
        rf.lo(a),
        rf.hi(a),
        rf.lo(b),
        rf.hi(b),
  ...
  @inline export function relaxed_min<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.relaxed_min<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.relaxed_min<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function relaxed_max<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.relaxed_max<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.relaxed_max<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function relaxed_q15mulr<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.relaxed_q15mulr<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.relaxed_q15mulr<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  @inline export function relaxed_dot<T>(dst: u32, a: u32, b: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.relaxed_dot<T>(v128.load(rf.addr(a)), v128.load(rf.addr(b))),
      );
      return;
    }
    store_lo_hi(
      dst,
      v128_swar.relaxed_dot<T>(rf.lo(a), rf.hi(a), rf.lo(b), rf.hi(b)),
    );
  }
  // @ts-expect-error: decorator
  ...
  @inline export function relaxed_trunc<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(rf.addr(dst), v128.relaxed_trunc<T>(v128.load(rf.addr(a))));
      return;
    }
  ...
  @inline export function relaxed_trunc_zero<T>(dst: u32, a: u32): void {
    if (ASC_FEATURE_RELAXED_SIMD) {
      v128.store(
        rf.addr(dst),
        v128.relaxed_trunc_zero<T>(v128.load(rf.addr(a))),
      );
      return;
    }
